...
data = [
    go.Scatter(x=M_range, y=avg_N_suits, name="Observed Suits", mode="markers"),
    go.Scatter(x=M_range, y=avg_N_valors, name="Observed Valors", mode="markers"),
    go.Scatter(x=M_range, y=Es, name="Expected Suits (recursion)", mode="lines"),
    go.Scatter(x=M_range, y=Ev, name="Expected Valors (recursion)", mode="lines"),
    go.Scatter(x=M_range, y=Es2, name="Expected Suits (formula)", mode="lines"),
    go.Scatter(x=M_range, y=Ev2, name="Expected Valors (formula)", mode="lines"),
]
layout = go.Layout(
    title="Playing Deck TTR Curve ({} trials)".format(ntrials),
    xaxis=dict(title="M (tokens, number of cards dealt)"),
    yaxis=dict(title="N (types, number of suits/valors held)"),
)
