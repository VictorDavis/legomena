...
# Fig 0: TTR comparison among authors
def graph0():
    data = [
        go.Scatter(
            x = books['tokens'],
...
app.layout = html.Div(children = [
    dcc.Graph(id = 'fig0', figure = graph0()), # TTR by author
    dcc.Dropdown(
        id = 'book-selector',
        options = [
    ...
    dcc.Graph(id = 'fig1'), # TTR Curve
    dcc.Graph(id = 'fig2') # Hapax & n-legomena proportions
])

# Fig 1: Actual vs Predicted types = f(tokens)
@app.callback(
    Output('fig1', 'figure'),
...
# Fig 2: Hapax & n-legomena proportions as sample size grows
@app.callback(
    Output('fig2', 'figure'),
    [ Input('book-selector', 'value') ]
)
