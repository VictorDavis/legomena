...
def plotTetrakis():
    ...
    data = [
        go.Scatter(x=mini["tokens"], y=mini["types"], name="Actual", mode="markers"),
        go.Scatter(
            x=mini["tokens"], y=mini["predicted"], name="Predicted", mode="lines"
        ),
    ]
    layout = go.Layout(
...
def plotWFD(filename="melville-moby_dick.txt"):
    ...
    data = [go.Scatter(x=df["rank"], y=df["freq"], mode="markers")]
    layout = go.Layout(
