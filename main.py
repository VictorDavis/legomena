...
def modelValues(m, Mz, Nz):
    x = m / Mz
    if math.fabs(x - 1) < 0.01:  # discontinuity @ m=M
        types = Nz
        hapax = Nz / 2
        dis = Nz / 6
        tris = Nz / 12
        ...
        tris = (
            Nz
            * (2 * x ** 4 - 6 * logx * x ** 3 + 3 * x ** 3 - 6 * x ** 2 + x)
            / 6
            / (x - 1) ** 4
        )
        tetrakis = (
...
def solveForX(H):
    ...
    dx = 0.5
    timer = 1
    epsilon = 0.00000001  # 10e-8 -> min ~27 iterations
...
def getTTRCurve(decks):
    ...
    for tokens in decks["tokens"].unique():
        ...
        nlegomena = [
            deck.loc[n, "ntypes"] if n in deck.index else 0 for n in range(0, 6)
        ]
        df.append(
            (
                tokens,
    ...
    df = pd.DataFrame(
        df, columns=["tokens", "types", "hapax", "dis", "tris", "tetrakis", "pentakis"]
    )
    return df
...
def calcRMSE(df):
    ...
    heaps = (
        math.sqrt(
            sum(
                [
                    (row["types"] - row["types_pred_heaps"]) ** 2
    ...
    iseries = (
        math.sqrt(
            sum(
                [
                    (row["types"] - row["types_pred_iseries"]) ** 2
                    for index, row in df.iterrows()
                ]
            )
            / n
    ...
    model = (
        math.sqrt(
            sum(
                [
                    (row["types"] - row["types_pred_model"]) ** 2
                    for index, row in df.iterrows()
                ]
            )
            / n
...
def predictInfSeries(df, M, N, deck):
    ...
    for tokens in df["tokens"]:
        ...
        for index, row in deck.iterrows():
            if last_term < 0.01:  # series should converge quickly
                break
            k = row["ntypes"]
            s = row["repeats"]
            x = 1 - tokens / M
...
def getDeck(fdist):
    deck = collections.Counter(
        fdist["freq"]
    )  # frequency distribution of frequencies {6:405} = 405 words appear exactly 6 times
...
def getBookStats(decks, ttr):
    ...
    for i, col in {
        ...
        ttr["{}_frac_pred".format(col)] = (
            ttr["{}_pred_model".format(col)] / ttr["types_pred_model"]
        )
    RMSE = calcRMSE(ttr)
